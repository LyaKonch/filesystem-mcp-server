...
class WindowsOSManager(
    ...
    @export_tool(name="backup_registry_keys", logger=logging.getLogger(__name__))
    async def backup_registry_keys(
        self, ctx: Context, hive: str, sub_key: str, output_path: str
    ) -> str:
    ...
    @export_tool(name="restore_registry_keys_from_file", logger=logging.getLogger(__name__))
    async def restore_backup(self, ctx: Context, file_path: str):
