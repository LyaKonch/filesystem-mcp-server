...
class ServiceManager(BaseServiceManager):
        ...
        def _sync():
            try:
                ...
                if e.winerror == winerror.ERROR_SERVICE_ALREADY_RUNNING:
                    ...
                    return f"Access Denied. Administrator privileges required to start '{service_name}'."
                else:
                    return f"Failed to start service: {getattr(e, 'strerror', str(e))}"
        ...
        def _sync():
            try:
                ...
                if e.winerror == winerror.ERROR_SERVICE_NOT_ACTIVE:
                    ...
                    return f"Access Denied. Administrator privileges required to stop '{service_name}'."
                else:
                    return f"Failed to stop service: {getattr(e, 'strerror', str(e))}"
    ...
    async def restart_service(self, service_name: str) -> str:
        ...
        def _sync():
            try:
                ...
                return f"Failed to restart service: {getattr(e, 'strerror', str(e))}"
    ...
    async def change_service_startup_type(self, service_name: str, startup_type: str) -> str:
        ...
        if startup_type not in mapping:
            return f"Invalid startup_type '{startup_type}'. Use one of {list(mapping.keys())}."
    ...
    async def change_service_config(
        ...
        if start_type:
            if start_type not in mapping:
                return f"Invalid start_type '{start_type}'. Use one of {list(mapping.keys())}."
        ...
        def _sync():
            ...
            try:
                ...
                return f"Failed to update config: {getattr(e, 'strerror', str(e))}"
            finally:
                try:
                    if hs:
    ...
    async def get_service_logs(
        ...
        def _sync():
            ...
            try:
                ...
                self.logger.warning(f"Failed to read event log via modern API: {e}")
    ...
    async def create_service(
        ...
        def _sync():
            ...
            try:
                ...
                return f"Failed to create service: {getattr(e, 'strerror', str(e))}"
            finally:
                try:
                    if hscm:
                        win32service.CloseServiceHandle(hscm)
    ...
    async def delete_service(self, service_name: str) -> str:
        """Delete an installed service. It's recommended to stop the service first if it's running."""
        ...
        def _sync():
            try:
                ...
                if e.winerror == winerror.ERROR_SERVICE_DOES_NOT_EXIST:
                    return f"Service '{service_name}' does not exist (or already deleted)."
                elif e.winerror == winerror.ERROR_SERVICE_MARKED_FOR_DELETE:
                    return f"Service '{service_name}' is already marked for deletion. It will disappear once stopped."
                elif e.winerror == 5:
                    return f"Access Denied. Administrator privileges required to delete '{service_name}'."
